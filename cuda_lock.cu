...
int main()
{
    ...
    cuda_status = cudaHostAlloc( (void**)&host_out_image,image_size*sizeof(short),cudaHostAllocDefault );
    if(cuda_status != cudaSuccess)
    {
    ...
    //copy out
    cuda_status = cudaMemcpy((short*)host_out_image,dev_out_image,sizeof(short)*image_size,cudaMemcpyDeviceToHost);
    if(cuda_status != cudaSuccess)
    {
        fprintf(stderr,"cudaMemcpy Failed");
        exit( EXIT_FAILURE );
    ...
    memcpy((short*)out_image.data,host_out_image,sizeof(short)*image_size);

    cudaFreeHost(host_out_image);
